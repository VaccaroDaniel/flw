...
$unitdir = 'D:/WinPro.Delta/Projects/FLW/FLW-V2/adventure-english-world/unit006';
$webcontentdir = $CFG->dirroot . '/flwcontent/english/adventure_v2/unit006_native';
$webcontenturl = $CFG->wwwroot . '/flwcontent/english/adventure_v2/unit006_native';
$shortname = 'FLW-AEW2-U006-NATIVE';
$fullname = 'Adventure English World V2 - Unit 6 Mini Quest Review';
$coursesummary = '<p>FLW Moodle-native course generated from unit006.</p>';
$targetcategoryid = 0;
...
body#page-course-view-topics.path-course-view .activity.label.modtype_label {
    margin:0!important;
}
...
body#page-course-view-topics.path-course-view .flw-native section.lesson:not(.lesson-fold) {
    background:#fff!important;
    border:3px solid #81b95b!important;
...
body#page-course-view-topics.path-course-view .flw-native section.lesson:not(.lesson-fold) > summary:first-child {
    cursor:default!important;
    list-style:none!important;
    display:block!important;
    margin:0!important;
    padding:18px 22px!important;
    background:linear-gradient(90deg,#e8f6db,#fff)!important;
...
body#page-course-view-topics.path-course-view .flw-native section.lesson:not(.lesson-fold) > summary:first-child::-webkit-details-marker {
    display:none!important;
}
body#page-course-view-topics.path-course-view .flw-native section.lesson:not(.lesson-fold) .lesson-body {
    padding:18px 22px!important;
}
...
function top_level_unit_blocks(string $html): array {
    ...
    $blocks = [];
    $offset = 0;
    while (preg_match('/<(header|section|details|article)\b[^>]*>/i', $content, $match, PREG_OFFSET_CAPTURE, $offset)) {
...
function summary_title_from_lesson(string $fragment, int $lessonnumber): string {
    ...
    $summaryhtml = preg_replace('/<span\b[^>]*\bclass=["\'][^"\']*\bplus\b[^"\']*["\'][^>]*>[\s\S]*?<\/span>/i', ' ', $summaryhtml);
    $summaryhtml = preg_replace('/<span\b[^>]*>\s*\d+\s*<\/span>/i', ' ', $summaryhtml, 1);
    $summary = clean_heading_text($summaryhtml);
    if ($summary === '') {
        return 'Lesson ' . $lessonnumber;
    }
    $title = preg_replace('/^Lesson\s+' . $lessonnumber . '\s*[—-]\s*/u', 'Lesson ' . $lessonnumber . ' - ', $summary);
...
function name_top_level_unit_block(string $fragment, int &$lessonnumber): string {
    ...
    if ($tag === 'details' && preg_match('/<summary[^>]*>([\s\S]*?)<\/summary>/i', $fragment, $summarymatch)) {
        $summaryhtml = preg_replace('/<span\b[^>]*\bclass=["\'][^"\']*\bplus\b[^"\']*["\'][^>]*>[\s\S]*?<\/span>/i', ' ', $summarymatch[1]);
        $summary = clean_heading_text($summaryhtml);
    }
...
function build_native_sections(string $sourcefile, string $webcontenturl): array {
    ...
    foreach ($blocks as $fragment) {
        ...
        $sections[] = ['section' => count($sections) + 1, 'name' => $name, 'html' => rewrite_fragment_for_native($fragment, $webcontenturl)];
    }
    return $sections;
}
...
$quizdata = add_quiz_activity($course, $quizsection, 'Unit 6 Mini Quest Review', '');
