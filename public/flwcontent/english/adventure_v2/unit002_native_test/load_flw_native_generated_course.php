...
$webcontentdir = $CFG->dirroot . '/flwcontent/english/adventure_v2/unit002_native_test';
$webcontenturl = $CFG->wwwroot . '/flwcontent/english/adventure_v2/unit002_native_test';
$shortname = 'FLW-AEW2-U002-NATIVE-TEST';
$fullname = 'NATIVE TEST - Adventure English World V2 - Unit 2 Color Quest';
...
function create_test_course_below_100(string $shortname, string $fullname, string $summary): stdClass {
    ...
    if ($old = $DB->get_record('course', ['shortname' => $shortname], '*', IGNORE_MISSING)) {
        if ((int)$old->id >= 100) {
            fail_now("Matching test shortname exists outside testing id range: {$old->id}");
        }
        delete_course($old, false);
        fix_course_sortorder();
    }
    $used = $DB->get_fieldset_select('course', 'id', 'id < 100', []);
    $used = array_map('intval', $used);
    $targetid = null;
    for ($candidate = 40; $candidate < 100; $candidate++) {
    ...
    if (!$targetid) {
        fail_now('No unused test course id below 100 is available.');
    }
    ...
    if ((int)$course->id >= 100) {
        fail_now("Created course id {$course->id}, expected testing id below 100.");
    }
...
$teacherhtml = '<div class="flw-native teacher-guide">' . file_get_contents($unitdir . '/teacher_guide.html') . '</div>';
