...
$unitdir = 'D:/WinPro.Delta/Projects/FLW/FLW-V2/adventure-english-world/unit003';
$webcontentdir = $CFG->dirroot . '/flwcontent/english/adventure_v2/unit003_native';
$webcontenturl = $CFG->wwwroot . '/flwcontent/english/adventure_v2/unit003_native';
$shortname = 'FLW-AEW2-U003-NATIVE';
$fullname = 'Adventure English World V2 Unit 3 - Number Trail';
$coursesummary = '<p>FLW Moodle-native course generated from unit003.</p>';
$targetcategoryid = 43;
...
function initNativeImageZoom() {
  document.querySelectorAll('.flw-native .zoom[data-img]').forEach(btn => {
    ...
    btn.addEventListener('click', event => {
      ...
      let caption = modal ? modal.querySelector('#modal-cap, .modal-title') : null;
      if (!modal || !image) {
...
function discover_source_css(string $unitdir): string {
    ...
    if (is_file($index)) {
        ...
        if ($html !== false && preg_match_all('/<style\b[^>]*>([\s\S]*?)<\/style>/i', $html, $matches)) {
...
.flw-native section.lesson {
    background:#fffdf7;
    border:2px solid #dfe7c9;
    border-radius:22px;
    margin:18px 0;
    padding:0!important;
    overflow:hidden;
    box-shadow:0 6px 18px rgba(24,72,42,.1);
}
.activity.label.modtype_label,
.activity.label.modtype_label .activity-item,
.activity.label.modtype_label .description,
.activity.label.modtype_label .contentwithoutlink,
.activity.label.modtype_label .no-overflow {
...
.flw-native section.lesson > summary {
    cursor:default;
    list-style:none;
    background:linear-gradient(90deg,#2d7f50,#53a765);
    color:#fff;
    margin:0!important;
    padding:18px 20px;
    width:100%;
...
function build_native_sections(string $sourcefile, string $webcontenturl): array {
    ...
    if (preg_match('/<header class="header" id="top">/i', $html)) {
        ...
        if ($story !== '') {
            $sections[] = ['section' => count($sections) + 1, 'name' => 'Unit Story', 'html' => rewrite_fragment_for_native($story, $webcontenturl)];
        }
        for ($i = 1; $i <= 7; $i++) {
            ...
            $sections[] = ['section' => count($sections) + 1, 'name' => $title, 'html' => rewrite_fragment_for_native($fragment, $webcontenturl)];
        }
        ...
        $sections[] = ['section' => count($sections) + 1, 'name' => 'Watch Lesson', 'html' => rewrite_fragment_for_native($watch, $webcontenturl)];
        $sections[] = ['section' => count($sections) + 1, 'name' => 'Final Project', 'html' => rewrite_fragment_for_native($project, $webcontenturl)];
        return $sections;
    }

    $html = preg_replace('/<nav\b[\s\S]*?<\/nav>\s*/i', '', $html, 1);
    $overview = first_match_or_empty($html, '/<(header|section)\b(?=[^>]*\bclass="(?:[^"]*\s)?hero(?:\s[^"]*)?")[^>]*>[\s\S]*?<\/\1>/i');
    $core = first_match_or_empty($html, '/<section\b(?=[^>]*\bclass="(?:[^"]*\s)?card(?:\s[^"]*)?")[^>]*>[\s\S]*?<\/section>/i');
    if ($overview === '') {
    ...
    if ($core !== '') {
        $sections[] = ['section' => count($sections) + 1, 'name' => 'Core Language', 'html' => rewrite_fragment_for_native($core, $webcontenturl)];
    }

    if (!preg_match_all('/<details\b(?=[^>]*\bclass="(?:[^"]*\s)?lesson(?:\s[^"]*)?")[^>]*>[\s\S]*?<\/details>/i', $html, $matches)) {
        fail_now('Could not extract lesson details blocks.');
    }
    $lessonnumber = 1;
    foreach ($matches[0] as $fragment) {
        $summary = '';
        if (preg_match('/<summary[^>]*>\s*<span[^>]*>([\s\S]*?)<\/span>/i', $fragment, $summarymatch)) {
            $summary = trim(preg_replace('/\s+/', ' ', strip_tags($summarymatch[1])));
        } else if (preg_match('/<summary[^>]*>([\s\S]*?)<\/summary>/i', $fragment, $summarymatch)) {
            $summary = trim(preg_replace('/\s+/', ' ', strip_tags($summarymatch[1])));
        }
        if (stripos($summary, 'Watch') === 0) {
            $sections[] = ['section' => count($sections) + 1, 'name' => 'Watch Lesson', 'html' => rewrite_fragment_for_native($fragment, $webcontenturl)];
            continue;
        }
        if (stripos($summary, 'Final Project') === 0 || stripos($summary, 'Project') !== false) {
            $sections[] = ['section' => count($sections) + 1, 'name' => 'Final Project', 'html' => rewrite_fragment_for_native($fragment, $webcontenturl)];
            continue;
        }
        if ($lessonnumber <= 7) {
...
$quizdata = add_quiz_activity($course, $quizsection, 'Unit 3 Number Trail', '');
