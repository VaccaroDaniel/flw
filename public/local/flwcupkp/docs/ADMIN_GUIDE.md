...
- `/local/flwcupkp/trace.php`: traceability report from Competency to Use Point, Knowledge Point, Moodle activity, evidence, and learner/class state.
...
- `/local/flwcupkp/performance.php?courseid=124&unitcode=U038`: reusable teacher scoring page for speaking, writing, and project performance evidence in any mapped unit.
- `/local/flwcupkp/performance_u038.php?courseid=124`: legacy focused U038 teacher scoring page.
...
- `/local/flwcupkp/calibration.php`: production evidence calibration report with active rule thresholds, evidence distributions, mastery outcomes, edge cases, JSON/CSV export, and saved snapshots for review.
- `/local/flwcupkp/calibration_proposal.php`: threshold calibration workflow that opens saved snapshots, previews proposed mastery threshold changes, saves draft proposals, activates reviewed calibrated rule versions, simulates affected learner-state recalculation, applies recalculation only after explicit admin confirmation, queues recalculation runs, and shows run history.
...
- Specialized Moodle evidence adapters are enabled for mapped quiz attempts, assignment submissions/grades, H5P scored statements, SCORM status/raw scores, activity completion, trusted STT results, manual evidence, and teacher performance scoring.
- External write web-service calls are rate-limited per user session and still require Moodle capabilities.
- Current U037/U038 fixtures map every Knowledge Point to at least one learning object, so production health should report 100% KP object coverage after import/linking.
- Teacher manual evidence is scoped to the selected course/unit filters.
- The generic performance page only exposes mapped performance tasks for the selected course/unit and stores rubric-backed teacher scores as KP, UP, or competency evidence.
- The U038 performance workflow is now available through the generic page; existing U038-only links remain available for compatibility.
...
- The generic unit teacher page now uses the shared unit report service to show KP evidence rows, parent UP/competency overview rows, and decision queue counts for any mapped unit code.
- The generic unit teacher page now supports KP evidence approval, KP state override, and KP override clearing for any mapped unit code. Decisions are audited and KP changes recalculate dependent UP/competency states.
...
- The generic `link_unit.php` CLI creates/reuses a Moodle shell course for an imported unit, creates page activities for mapped learning objects, links object `courseid`/`cmid`, and enrols deterministic teacher/student smoke-test accounts.
...
- The evidence calibration page lets admins filter by course, unit, and target type before reviewing provisional thresholds, score bands, performance-evidence coverage, low-confidence states, review-due rows, and suspicious state/evidence mismatches. Admins can export the current report as JSON/CSV, save named snapshots, and compare the current summary against the latest matching snapshot before threshold changes.
- Threshold proposals are based on saved calibration snapshots. Draft proposals do not alter learner states; activation writes a new active calibrated rule version for the selected target type. After activation, admins can preview exactly which existing learner states would change, then run controlled recalculation immediately or queue it for scheduled processing in the affected snapshot scope. Future direct evidence calculations load the calibrated rule before falling back to built-in provisional defaults.
