<?php
// CLI for Program 3 Gate E3 Retention / Retrieval / Review.
...
if (!empty($options['help'])) {
    echo "C-UP-KP retention/retrieval/review service\n\n";
    echo "Options:\n";
    ...
    echo "Examples:\n";
    echo "  php local/flwcupkp/cli/retention_review.php --action=status --courseid=124 --unitcode=U038\n";
    echo "  php local/flwcupkp/cli/retention_review.php --action=preview --courseid=124 --unitcode=U038\n";
    echo "  php local/flwcupkp/cli/retention_review.php --action=apply --courseid=124 --unitcode=U038 --confirm=1\n";
    exit(0);
}
...
try {
    if ($action === 'status') {
        $result = \local_flwcupkp\local\retention_review_service::status($courseid, $unitcode, $frameworkid, $limit);
    } else if ($action === 'learner') {
        if ($userid <= 0) {
            cli_error('Learner action requires --userid=ID.');
        }
        $result = \local_flwcupkp\local\retention_review_service::current_retention_state(
            $userid,
            $courseid,
            $unitcode,
            $frameworkid,
        ...
        $result = \local_flwcupkp\local\retention_review_service::class_summary($courseid, $unitcode, $frameworkid,
            $limit);
