...
/**
 * Moodle event observers for FLW Exam.
 */
class observer {
    /**
     * Sync a linked Moodle Quiz attempt after Moodle submits it.
     *
    ...
    /**
     * Sync a linked Moodle Quiz attempt after Moodle grades it.
     *
    ...
    /**
     * Sync a linked Moodle Quiz attempt after manual grading completes.
     *
    ...
    /**
     * Import the event attempt into every visible FLW Exam linked to the quiz.
     *
    ...
    protected static function sync_quiz_attempt(\core\event\base $event, string $sourceevent): void {
        ...
        foreach ($exams as $exam) {
            try {
                exam_service::record_quiz_attempt_result_from_event((int)$exam->id, $userid, $attemptid);
            } catch (\Throwable $e) {
                debugging(
                    'local_flwexam could not sync Moodle Quiz attempt ' . $attemptid . ' from ' . $sourceevent .
                        ': ' . $e->getMessage(),
                    DEBUG_DEVELOPER
                );
            }
